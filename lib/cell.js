...
util.inherits(SporeCell, SporeLocalOnly);
...
SporeCell.prototype._remoteSave = function (contents, callback) {
  ...
  this.addToLocalOnly(function (err) {
    ...
    self.api(function (err, api) {
      if(err) return callback(err);

      debug("Sending API request to create cell for " + this.fullName());

      api.createCell(self.remotePath(), contents, function (err) {
        if(err && Errors.noConnection.test(err)) {
          debug("Can't save " + this.fullName() + " to remote due to lack of a connection. We'll try again later.");
          return callback();
        }
...
SporeCell.prototype._load = function (callback) {
  ...
  this._localLoad(function (err, json) {
    if(err && Errors.noCache.test(err)) {
      return self._remoteLoad(self._handleLoad(callback));
    }

    self._handleLoad(callback)(err, json);
  });
};
...
SporeCell.prototype._remoteLoad = function (callback) {
  ...
  callback(Errors.noCache.build(this.fullName()));
};

SporeCell.prototype._handleLoad = function (callback) {
  var self = this;
...
SporeCell.prototype._loadJson = function (json) {
