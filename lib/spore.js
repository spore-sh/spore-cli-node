...
Spore.prototype.signup = function (email, password, callback) {
  ...
  this.api(function (err, api) {
    ...
    api.signup(email, password, function (err, user) {
      ...
      try {
        self.setKey(api, user.email, user.key);
      } catch(e) {
        return callback(e);
      }
...
Spore.prototype.login = function (email, password, callback) {
  ...
  this.api(function (err, api) {
    ...
    api.signup(email, password, function (err, key) {
      if(err) return callback(err);

      try {
        self.setKey(api, email, key);
      } catch(e) {
        return callback(e);
      }
...
Spore.prototype.setKey = function (email, key, callback) {
  ...
  async.parallel({
    ...
    try {
      ...
      if(!netrc.machines[config.host]) {
        netrc.addMachine(config.host, {});
      }

      netrc.host(config.host).login = email;
      netrc.host(config.host).password = key;
...
Spore.prototype.getKey = function (callback) {
  ...
  async.parallel({
    ...
    try {
      ...
      email = netrc.host(this.host).email;
    } catch(e) {
      return callback(e);
    }
