...
function Spore() {
  ...
  SporeLookup.call(this);
}

util.inherits(Spore, SporeConfig);
util.inherits(Spore, SporeLookup);

Spore.App = SporeApp;

Spore.prototype.create = function (dir, callback) {
  var self = this;
  ...
  this.load(dir, function (err, app) {
    ...
    self.lookupName(dir, function (err, name) {
      if(err) return callback(err);

      calback(null, new self.constructor.App({ name: name }));
    });
  });
};

Spore.prototype.load = function (dir, callback) {
  var self = this;

  this.sporeFile(function (err, sporeFile) {
    if(err) return callback(err);
    ...
    fs.readJson(sporeFilePath, { encoding: 'utf8' }, function (err, json) {
      ...
      callback(null, new self.constructor.App(json));
    });
  });
};
