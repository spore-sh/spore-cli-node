...
function Spore() {
  this.configDir = process.env.SPORE_HOME || resolvePath('~/.spore');
  this.config = {};
  this.defaultConfig = {
    host: 'pod.spore.sh',
    sporeFile: '.spore',
...
Spore.prototype.getConfig = function (callback) {
  var configFile = resolvePath(this.configDir, 'config.json'),
      self = this;

  debug("Reading config file at " + configFile);

  fs.readFile(configFile, { encoding: 'utf8' }, function (err, raw) {
    if(err && err.code === 'ENOENT') {

      debug("No config file found, creating one.");

      fs.ensureDir(path.dirname(configFile), function (err) {
        if(err) return callback(err);

        fs.writeFile(configFile, JSON.stringify(self.defaultConfig, null, 2), function (err) {
          if(err) return callback(err);

          debug("Default config written to " + configFile);
