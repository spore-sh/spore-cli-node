  ...
  async.parallel({
    ...
    try {
      if(!netrc.hasHost(config.host.hostname)) {
        debug(".netrc does not have a host entry for " + config.host.hostname + " - adding a new one");
...
Spore.prototype.getKey = function (callback) {
  ...
  async.parallel({
    ...
    try {
      if(netrc.hasHost(config.host.hostname)) {
        debug(".netrc contains an entry for " + config.host.hostname + " - retrieving");

        key = netrc.host(config.host.hostname).password;
        email = netrc.host(config.host.hostname).login;
      } else {
        debug(".netrc contains no entry for " + config.host.hostname);
      }
    } catch(e) {
      return callback(e);
    }
